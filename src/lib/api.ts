...
class ApiService {
  ...
  async generateTickets(prompt: string): Promise<TicketGenerationResponse> {
    ...
    return this.request<TicketGenerationResponse>('/generate-tickets', {
      method: 'POST',
  ...
  async getTickets(): Promise<TicketsResponse> {
    return this.request<TicketsResponse>('/tickets');
  }
  ...
  async updateTicketStatus(
    ...
    return this.request<TicketUpdateResponse>(`/tickets/${ticketId}/status`, {
      method: 'PUT',
      body: JSON.stringify(body),
    });
  }
  ...
  async clearAllTickets(): Promise<void> {
    await this.request<void>('/tickets', {
      method: 'DELETE',
    });
  }
}
